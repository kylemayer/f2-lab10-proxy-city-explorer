...
const { mungeLocationRes, mungeWeatherRes } = require('./munging.js');
...
app.get('/weather', async(req, res) => {
  try {
    const data = await request.get(`https://api.weatherbit.io/v2.0/forecast/daily?lat=${req.query.latitude}&lon=${req.query.ongitude}&key=${process.env.WEATHER_KEY}`);
